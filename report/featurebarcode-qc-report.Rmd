...
pretty_log10_breaks <- function () {
    function(x) {
        ...
        breaks = 10^rep(bottom_10:top_10, each = 3) * c(1, 2, 
            5)
    }
}
...
pdna_gg <- read_stats %>% 
    ...
    scale_y_log10_pretty(
        limits  = pretty_log10_limits(read_stats$count)) +
    labs(y = "Count",
         title = "pDNA reads") +
...
# feature_counts <- ldply(feature_count_files, read_tsv, .id = "sample")
